...
const LEGACY_TABLE = "squad_presets";
...
const buildPresetsFromTeams = async (
  teams: TeamRow[]
): Promise<SquadPreset[]> => {
...
const fetchLegacyPresets = async () => {
  if (!supabase) {
    return [] as SquadPreset[];
  }
  const { data } = await supabase
    .from(LEGACY_TABLE)
...
export const fetchSquadPresets = async () => {
  ...
  if (!teamError && (teamRows ?? []).length > 0) {
    const presets = await buildPresetsFromTeams((teamRows ?? []) as TeamRow[]);
    return { ok: true as const, presets };
  }

  // Backward compatibility fallback while old rows still exist.
  if (teamError) {
    const legacy = await fetchLegacyPresets();
    if (legacy.length > 0) {
      return { ok: true as const, presets: legacy };
    }
    return { ok: false as const, error: teamError.message };
  }

  return { ok: true as const, presets: [] as SquadPreset[] };
};
...
export const createSquadPreset = async (payload: {
  ...
  const presets = await buildPresetsFromTeams([team as TeamRow]);
  if (presets.length === 0) {
    return { ok: false as const, error: "Could not fetch saved team." };
  }

  return { ok: true as const, preset: presets[0] };
};
...
export const updateSquadPreset = async (payload: {
  ...
  const presets = await buildPresetsFromTeams(updatedTeams);
